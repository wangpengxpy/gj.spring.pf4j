...
package gj.pf4j.migration.kingbasees;
...
public enum KingbaseESDialect implements Dialect {

    INSTANCE;

    private static final Logger log = LoggerFactory.getLogger(KingbaseESDialect.class);
    ...
    @Override public DbType dbType() { return DbType.KingbaseES; }
    ...
    public String renderPrimaryKey(DatabasePrimaryKey pk) {
        ...
        String def = quoteIdentifier(col.getName()) + " " + col.getStoreType()
                + " GENERATED BY DEFAULT AS IDENTITY (INCREMENT 1 MINVALUE 1 MAXVALUE 2147483647 START 1)"
                + " NOT NULL PRIMARY KEY";
        log.debug("[{}] PK: storeType={} → {}", col.getName(), col.getStoreType(),
    ...
    public DdlStatement renderCreateTable(DatabaseTable table) {
        ...
        log.debug("[{}] CREATE SQL (KingbaseES):\n{}", table.getName(), sql);
