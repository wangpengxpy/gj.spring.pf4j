...
public enum PostgreSqlDialect implements Dialect {
    ...
    @Override public DbType dbType() { return DbType.PostgreSQL; }
    ...
    public String renderPrimaryKey(DatabasePrimaryKey pk) {
        ...
        log.debug("[{}] PK: storeType={} → {}",
                col.getName(), col.getStoreType(), "GENERATED BY DEFAULT AS IDENTITY");
        return def;
    }
    ...
    public DdlStatement renderCreateTable(DatabaseTable table) {
        ...
        log.debug("[{}] CREATE SQL (PostgreSQL):\n{}", table.getName(), sql);
